...
def get_employees(db: Session, skip: int = 0, limit: int = 100, department: str | None = None):
    q = db.query(Employee)
    if department:
...
def get_attendance(db: Session, employee_id: int | None = None, skip: int = 0, limit: int = 100):
...
def update_attendance(db: Session, record_id: int, data: AttendanceUpdate) -> Attendance | None:
...
def delete_attendance(db: Session, record_id: int) -> bool:
    record = db.query(Attendance).filter(Attendance.id == record_id).first()
    if not record:
        return False
    db.delete(record)
...
def create_payroll(db: Session, data: PayrollCreate) -> Payroll:
    d = data.model_dump()
    # Auto-calculate net_salary if not supplied
...
def get_payrolls(db: Session, employee_id: int | None = None, skip: int = 0, limit: int = 100):
    q = db.query(Payroll)
    if employee_id:
...
def update_payroll(db: Session, record_id: int, data: PayrollUpdate) -> Payroll | None:
    record = db.query(Payroll).filter(Payroll.id == record_id).first()
    if not record:
        return None
    for key, value in data.model_dump(exclude_unset=True).items():
        setattr(record, key, value)
...
def delete_payroll(db: Session, record_id: int) -> bool:
    record = db.query(Payroll).filter(Payroll.id == record_id).first()
    if not record:
        return False
    db.delete(record)
...
def create_performance(db: Session, data: PerformanceCreate) -> Performance:
    record = Performance(**data.model_dump())
    db.add(record)
    db.commit()
    db.refresh(record)
    return record
...
def get_performances(db: Session, employee_id: int | None = None, skip: int = 0, limit: int = 100):
...
def update_performance(db: Session, record_id: int, data: PerformanceUpdate) -> Performance | None:
    record = db.query(Performance).filter(Performance.id == record_id).first()
    if not record:
        return None
    for key, value in data.model_dump(exclude_unset=True).items():
        setattr(record, key, value)
...
def delete_performance(db: Session, record_id: int) -> bool:
    record = db.query(Performance).filter(Performance.id == record_id).first()
    if not record:
        return False
    db.delete(record)
