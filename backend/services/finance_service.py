...
def get_expenses(db: Session, skip: int = 0, limit: int = 100, category: str | None = None):
    q = db.query(Expense)
    if category:
...
# Revenue CRUD
# ═══════════════════════════════════════════════════════════════════════════
def create_revenue(db: Session, data: RevenueCreate) -> Revenue:
    record = Revenue(**data.model_dump())
    db.add(record)
    db.commit()
    db.refresh(record)
    return record
...
def get_revenues(db: Session, skip: int = 0, limit: int = 100, source: str | None = None):
    q = db.query(Revenue)
    if source:
...
def get_revenue(db: Session, revenue_id: int) -> Revenue | None:
    return db.query(Revenue).filter(Revenue.id == revenue_id).first()


def update_revenue(db: Session, revenue_id: int, data: RevenueUpdate) -> Revenue | None:
...
# ═══════════════════════════════════════════════════════════════════════════
def total_expenses(db: Session, year: int | None = None) -> float:
...
def total_revenue(db: Session, year: int | None = None) -> float:
...
def monthly_revenue(db: Session, year: int):
    ...
    rows = (
        ...
        .filter(func.extract("year", Revenue.date) == year)
...
def monthly_expenses(db: Session, year: int):
    """Return list of (month, total) tuples for the given year."""
    rows = (
        db.query(
...
def expenses_by_category(db: Session, year: int | None = None):
